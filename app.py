...
def timer_json(timer_id):
    ...
    if running:
        ...
        if remaining == 0:
            running = False
            with db() as conn:
                conn.execute("UPDATE timers SET remaining=0,running=0,updated_at=? WHERE id=?", (int(time.time()), timer_id))
                conn.commit()
...
def normalize_slug(value):
    ...
    return "-".join(part for part in value.replace("/", " ").split() if part)[:80]
...
def new_timer():
    ...
    with db() as conn:
        conn.execute("INSERT INTO timers(id,duration,remaining,running,updated_at) VALUES(?,?,?,?,?)", (timer_id, 300, 300, 0, int(time.time())))
        conn.commit()
...
def timer_page(timer_id):
    if not timer_json(timer_id):
        return "Таймер не найден", 404
...
def timer_action(timer_id, action):
    ...
    if action == "start":
        ...
        try:
            remaining += max(1, int(request.args.get("seconds", 0)))
        except ValueError:
            return jsonify({"error": "Неверное количество секунд"}), 400
    else:
        return jsonify({"error": "Неизвестное действие"}), 400
    with db() as conn:
        conn.execute("UPDATE timers SET remaining=?,running=?,updated_at=? WHERE id=?", (remaining, int(running), now, timer_id))
        conn.commit()
...
def create_poll():
    ...
    with db() as conn:
        ...
        conn.executemany("INSERT INTO options(poll_id,text) VALUES(?,?)", [(poll_id, x) for x in options])
        conn.commit()
